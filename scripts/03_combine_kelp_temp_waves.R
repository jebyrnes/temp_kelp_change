...
dat_year_filter <- . %>%
  filter(Year >= 1952 & Year <= 2008) #first data point in kelp, last da
...
wave_data_full <- left_join(unique_lat_long_tab, wave_data) %>%
  ...
  dat_year_filter %>%
  select(!contains("sd_")) #meh. not going to work with these, & want to minimize columns

##### 3) Calculate averages for each lat/long during the entire dataset period
wave_data_avg <- wave_data_full %>%
  group_by(Longitude, Latitude, LonGOW, LatGOW) %>%
  summarize(across(mean_wave_energy:q95_wave_height,
                   ~mean(.x, na.rm=TRUE),
                   .names = "{.col}_site")) %>%
  ungroup()

##### 4) Load and merge with wave/temp data
kelp_temp_data <- read.csv("derived_data/raw_data_with_temp.csv") 
...
rd_wave <- list(
  ...
  mutate(across(mean_wave_energy:q95_wave_height,
                ~.x - get(glue::glue("{cur_column()}_site")),
                .names = "{.col}_dev"))


##### 6) Write it all out
write_csv(rd_wave,  "derived_data/raw_data_with_temp_waves.csv")
